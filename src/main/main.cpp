...
const char *wifiData[][2] = {
    {"CMCC-5D7J", "wu1209932698"}, // 替换为自己常用的wifi名和密码
    // {"222", "12345678"},
    // 继续添加需要的 Wi-Fi 名称和密码
};

String APPID = "888c73ae"; // 自己的星火大模型账号参数
String APIKey = "78e18a1397b3c14cec3f3edc084f7a06";
String APISecret = "MWVjOGM1NjdjZmRiNjIxYWI2OWIzZjg1";
...
//ConnServer、ConnServer1：连接 WebSocket 服务器。
void ConnServer();
void ConnServer1();
...
const char *appId1 = "888c73ae"; // 替换为自己的星火大模型参数
...
onMessageCallback函数处理聊天服务的结果，接收模型生成的回答，并通过audio2进行语音合成，将文本转换为语音进行播放。*/
...
//onEventsCallback：处理 WebSocket 事件。
...
onMessageCallback1函数处理语音识别的结果。它从WebSocket接收到语音识别的文本数据，并根据文本内容采取相应的操作，例如播放音乐或进行聊天对话。*/
...
//onEventsCallback1：处理 WebSocket 事件。
...
//WebSocket 连接:
/*ConnServer和ConnServer1函数用于建立与指定WebSocket服务器的连接。webSocketClient用于与聊天服务进行通信，而webSocketClient1用于语音识别服务。*/
void ConnServer()
{
...
//wifiConnect() 函数用于连接到预定义的 Wi-Fi 网络。
//前为wifi名字后面为密码wifiData
//int numNetworks: 这是 wifiData 数组的行数，即要尝试连接的 WiFi 网络数量。
...
/*
Serial 是 Arduino 和一些其他嵌入式开发平台中的一个对象，用于与串口通信。它常用于调试和显示信息。你可以通过以下几种方式查看 Serial 输出：

Arduino IDE 的串口监视器:

在 Arduino IDE 中，你可以打开串口监视器来查看 Serial 输出。点击 IDE 顶部的“工具”菜单，然后选择“串口监视器”（或按 Ctrl+Shift+M）。
确保串口监视器的波特率设置与你的代码中的 Serial.begin() 函数中的波特率匹配（比如 9600）。
串口调试工具:

如果你使用的是其他开发平台或工具链，你可以使用类似 PuTTY、Tera Term、CoolTerm 等串口调试工具来连接到开发板的串口。
这些工具通常需要配置正确的 COM 端口和波特率来查看串口输出。
开发板上的 USB 端口:

大多数现代开发板（如 Arduino、ESP8266、ESP32）都有一个 USB 端口，连接到计算机后可以通过串口监视器工具查看串口数据。
*/
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            //堆内存（Heap Memory）是计算机内存的一部分，用于动态分配内存。当你在程序运行时需要申请内存来存储数据（如对象、数组、缓冲区等），堆内存提供了一种灵活的方式来满足这些需求。
            return; // 如果连接成功，退出函数
        }
    }
}
...
getUrl函数生成用于API请求的签名，确保请求的合法性和安全性。
*/
...
//时间同步:getTimeFromServer函数通过向百度发送请求来获取服务器时间，用于生成签名时的时间戳。
