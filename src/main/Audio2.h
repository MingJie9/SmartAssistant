...
成员函数：
AudioBuffer(size_t maxBlockSize = 0);：构造函数，创建一个音频缓冲区。
~AudioBuffer();：析构函数，释放缓冲区。
size_t init();：初始化缓冲区设置默认值。
bool isInitialized();：检查是否已初始化。
void setBufsize(int ram, int psram);：设置缓冲区大小。
void changeMaxBlockSize(uint16_t mbs);：更改最大块大小。
uint16_t getMaxBlockSize();：获取最大块大小。
size_t freeSpace();：返回可写入的空闲字节数。
size_t writeSpace();：返回从写指针到缓冲区末尾的空间。
size_t bufferFilled();：返回已填充的字节数。
void bytesWritten(size_t bw);：更新写指针。
void bytesWasRead(size_t br);：更新读指针。
uint8_t *getWritePtr();：返回当前写指针。
uint8_t *getReadPtr();：返回当前读指针。
uint32_t getWritePos();：获取相对写位置。
uint32_t getReadPos();：获取相对读位置。
void resetBuffer();：重置缓冲区。
bool havePSRAM();：检查是否有PSRAM。
成员变量：

size_t m_buffSizePSRAM：PSRAM中的缓冲区大小。
size_t m_buffSizeRAM：RAM中的缓冲区大小。
uint8_t *m_buffer：缓冲区指针。
uint8_t *m_writePtr：写指针。
uint8_t *m_readPtr：读指针。
uint8_t *m_endPtr：缓冲区末尾指针。
Audio2 类
功能：扩展音频处理功能，管理音频播放、流媒体连接、音量控制等

成员函数：
Audio2(bool internalDAC = false, uint8_t channelEnabled = 3, uint8_t i2sPort = I2S_NUM_0);：构造函数，初始化音频设置。
~Audio2();：析构函数。
void setBufsize(int rambuf_sz, int psrambuf_sz);：设置缓冲区大小。
bool connecttohost(const char *host, const char *user = "", const char *pwd = "");：连接到主机。
bool connecttospeech(const char *speech, const char *lang);：连接到语音服务。
bool connecttoFS(fs::FS &fs, const char *path, int32_t resumeFilePos = -1);：连接到文件系统。
bool connecttoSD(const char *path, int32_t resumeFilePos = -1);：连接到SD卡。
bool setFileLoop(bool input);：设置文件循环播放。
void setConnectionTimeout(uint16_t timeout_ms, uint16_t timeout_ms_ssl);：设置连接超时时间。
bool setAudioPlayPosition(uint16_t sec);：设置音频播放位置。
bool setFilePos(uint32_t pos);：设置文件位置。
bool audioFileSeek(const float speed);：调整音频文件的播放速度。
bool setTimeOffset(int sec);：设置时间偏移。
bool setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN = I2S_PIN_NO_CHANGE, int8_t MCK = I2S_PIN_NO_CHANGE);：设置I2S引脚。
bool pauseResume();：暂停或恢复播放。
bool isRunning();：检查播放状态。
void loop();：主循环函数。
uint32_t stopSong();：停止歌曲播放。
void forceMono(bool m);：强制单声道播放。
void setBalance(int8_t bal = 0);：设置音量平衡。
void setVolumeSteps(uint8_t steps);：设置音量步进。
void setVolume(uint8_t vol, uint8_t curve = 0);：设置音量。
uint8_t getVolume();：获取音量。
uint8_t maxVolume();：获取最大音量。
uint8_t getI2sPort();：获取I2S端口。
成员变量：
WiFiClient client; 和 WiFiClientSecure clientsecure;：用于网络连接的客户端。
SemaphoreHandle_t mutex_audio;：音频处理的信号量。
i2s_config_t m_i2s_config; 和 i2s_pin_config_t m_pin_config;：I2S配置。
std::vector<char *> m_playlistContent; 和 std::vector<char *> m_playlistURL;：播放列表缓冲区。
const size_t m_frameSizeWav、m_frameSizeMP3 等：不同音频格式的帧大小。
这段代码主要用于处理音频数据流、播放列表管理、音量控制和与网络的交互。AudioBuffer 类负责缓冲区的管理，而 Audio2 类扩展了更多功能，如音频播放控制和流媒体处理。
*/